...
<form action="allNotesServlet?sortId" method="post">
    <input type="hidden" name="sortId" value="0">
    <button type="submit">Sort by Date Added</button>
</form>
...
<ul>
    ...
    <div style="border: 1px solid blueviolet; padding: 10px; margin-bottom: 20px;">
        ...
        <form id="newTitleForm_<%=name%>" action="editNoteServlet?noteId=<%=name%>&editType=<%=0%>" method="post" style="display:none;">
            <label type="hidden" for="newContent_<%=name%>">Rename</label>
            <input id="newContent_<%=name%>" name="newContent" type="text">
            <button type="submit">
                Submit
            </button>
        </form>
        ...
        <p id="textBox_<%=name%>" onclick="document.getElementById('editTextForm_<%=name%>').style.display='block'"><%=text%> <a href="<%=url%>"><%=url%></a> </p>
        <form id="editTextForm_<%=name%>" action="editNoteServlet?noteId=<%=name%>&editType=<%=1%>" method="post" style="display:none;">
            <label type="hidden" for="newContent_<%=name%>">Edit:</label>
            <input id="editText_<%=name%>" name="newContent" type="text">
            <button type="submit">
                Submit
            </button>
        </form>
